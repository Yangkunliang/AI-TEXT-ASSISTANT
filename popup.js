      ...
      func: async () => {
        ...
        container.onmouseup = () => {
          ...
          if (selectedRect && selectedRect.width > 10 && selectedRect.height > 10) {
            tipText.textContent = `已选择区域: ${Math.round(selectedRect.width)} × ${Math.round(selectedRect.height)} 像素`;
  ...
  try {
    ...
    await chrome.scripting.executeScript({
      ...
      func: (url) => {
        ...
        const qrCodeUrl = generateSimpleQR(text);
        ...
        const truncatedUrl = text.length > 50 ? text.substring(0, 50) + '...' : text;
